...
class IndianTextPreprocessor:
    ...
    def clean_text(self, text: str, language: IndianLanguage = IndianLanguage.HINDI) -> Tuple[str, float]:
        ...
        # Apply Hindi-specific abbreviations only for Hindi
    ...
    def detect_language(self, text: str) -> str:
        ...
        try:
            ...
            return lang_map.get(lang, 'hi')  # Default to Hindi if undetected
        except:
...
class IndianTTS:
    ...
    async def validate_voice(self, voice: str, language: IndianLanguage) -> str:
        """Validate if the voice is supported by edge_tts, fallback to default if invalid."""
    ...
    def _get_default_voice(self, language: IndianLanguage) -> str:
        """Get default voice for the language based on gender."""
        voice_map = {
    ...
    def speak(self, text: str, language: IndianLanguage = IndianLanguage.HINDI, voice: str | None = None) -> Tuple[bool, LatencyMetrics]:
        try:
            loop = asyncio.get_running_loop()
            return loop.run_until_complete(self._speak_async(text, language, voice))
        except RuntimeError:
            return asyncio.run(self._speak_async(text, language, voice))
...
async def list_available_voices():
    """List available Edge TTS voices for Indian languages"""
    try:
        import edge_tts
...
def interactive_demo():
    ...
    while True:
        try:
            ...
            start_time = time.time()
            success, metrics = tts.speak(text, language, voice)
            ...
            print(f"❌ त्रुटि: {e} | Error: {e}")
            print("कृपया सुनिश्चित करें कि ffmpeg, pygame, और langdetect इंस्टॉल हैं | Ensure pygame, ffmpeg, and langdetect are installed")
